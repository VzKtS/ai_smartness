...
def _deterministic_hash(word: str) -> int:
    """
    Deterministic hash function that produces the same result across Python sessions.

    Python's built-in hash() is randomized since Python 3.3 for security,
    which breaks embedding compatibility across daemon restarts.
    """
...
class EmbeddingManager:
    ...
    def _embed_tfidf(self, text: str) -> List[float]:
        ...
        for word, count in tf.items():
            # Deterministic hash to get index (consistent across Python sessions)
            idx = _deterministic_hash(word) % 384
            # TF value (normalized)
            value = count / total
